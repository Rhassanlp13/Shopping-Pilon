...
async function loadUserRole() {
    ...
    if (!data) {
        ...
        const { error: insertError } = await supabase
            .from('profiles')
            .upsert({ id: currentUser.id, email: currentUser.email, role: 'customer' }, { onConflict: 'id' });
        if (insertError) {
...
async function handleLogin() {
    ...
    try {
        ...
        // No recargamos aquí; el listener de auth actualizará la UI
        // Si quieres recargar para asegurar, descomenta:
        // location.reload();
    } catch (err) {
...
export function bindAuthEvents() {
    ...
    // Solo asignar evento si no hay usuario logueado (la UI de logout ya maneja su propio click)

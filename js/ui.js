...
export const UI = {
    ...
    async abrirDetalle(id) {
        ...
        document.getElementById('detalle-img').src = esc(p.imagen);
        document.getElementById('detalle-img').alt = esc(p.nombre);
        ...
        baseCard.innerHTML = `
            ...
            <div class="variante-info">
                <div class="variante-nombre">${esc(p.nombre)}</div>
                <div class="variante-precio">$${Number(p.precio).toLocaleString('es-CU')}</div>
                <div class="variante-stock">${p.stock > 0 ? `${p.stock} disponibles` : 'Agotado'}</div>
            </div>
        `;
        ...
        if (p.variantes && p.variantes.length > 0) {
            p.variantes.forEach((v, i) => {
                ...
                const precioVar = v.precio ? `$${v.precio.toLocaleString('es-CU')}` : '';
                const card = document.createElement('div');
                card.className = 'variante-card';
                card.dataset.index = i;
                card.innerHTML = `
                    <div class="variante-imagen">
                        <img src="${esc(imagenVar)}" alt="${esc(v.nombre)}" 
                             onerror="this.src='https://placehold.co/80x80?text=?'">
                    </div>
                    <div class="variante-info">
                        <div class="variante-nombre">${esc(v.nombre)}</div>
                        ${precioVar ? `<div class="variante-precio">${precioVar}</div>` : ''}
                        <div class="variante-stock">${v.stock > 0 ? `${v.stock} disponibles` : 'Agotado'}</div>
                    </div>
                `;
    ...
    async guardarPedido(nombreCliente, telefonoCliente, items, telefonoVendedor, nombreVendedor) {
        const total = items.reduce((sum, it) => sum + it.precio * it.cantidad, 0);
        const seller_id = items[0]?.seller_id;
        if (!seller_id) {
            console.warn('No seller_id found for order');
            return null;
        }
    ...
    async enviarWhatsApp() {
        ...
        let valido = true;
        if (!nombre) {
            document.getElementById('g-cliente-nombre')?.classList.add('error');
            valido = false;
        } else {
            document.getElementById('g-cliente-nombre')?.classList.remove('error');
        }
        if (!telefono) {
            document.getElementById('g-cliente-tel')?.classList.add('error');
            valido = false;
        } else {
            document.getElementById('g-cliente-tel')?.classList.remove('error');
        }
        if (!valido) return;
        ...
        vendedores.forEach((grupo, i) => {
            ...
            if (pedidosIds[i]) {
                texto += `\n🔗 *Confirmar pedido:* https://shopping-pilon.pages.dev/confirmar.html?pedido=${pedidosIds[i]}`;
            }
        ...
        setTimeout(() => {
            ...
            this.renderProductos();
            this.cerrarModal();
            this.mostrarToast(`✅ Pedido enviado a ${vendedores.length} vendedor${vendedores.length > 1 ? 'es' : ''}`, 'ok');
    ...
    bindEventos() {
        ...
        document.getElementById('cliente-nombre')?.addEventListener('input', () => {
            document.getElementById('g-cliente-nombre')?.classList.remove('error');
        });
        ...
        this.cartItems?.addEventListener('click', e => {
            ...
            if (btnA && !btnA.disabled) {
                ...
                carrito.aumentar(btnA.dataset.id, variantId, this.productos);
                this.actualizarContador();
                this.renderCarrito();
                this.renderProductos();
            }
            if (btnD) {
                ...
                this.renderCarrito();
                this.renderProductos();
                if (carrito.cantidad === 0) this.mostrarToast('🛒 Carrito vacío', 'info');
            }
        });

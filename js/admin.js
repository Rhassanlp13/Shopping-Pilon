...
let editandoId  = null;
let borrandoId  = null;
let iniciado    = false; // ← fix doble inicialización
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function mostrarShell() {
        ...
        if (!iniciado) { // ← evita doble llamada
            iniciado = true;
            iniciar();
        }
    }
    ...
    // ── Registro de vendedor (modal, no prompt) ───────────────────────────────
    document.getElementById('btn-registro')?.addEventListener('click', () => {
        document.getElementById('modal-registro-admin')?.removeAttribute('hidden');
    });

    document.getElementById('close-registro-admin')?.addEventListener('click', () => {
        document.getElementById('modal-registro-admin')?.setAttribute('hidden', '');
    });

    document.getElementById('btn-registrar-admin')?.addEventListener('click', async () => {
        ...
        const btn = document.getElementById('btn-registrar-admin');
        btn.disabled = true;
        ...
        if (error) {
            ...
            document.getElementById('modal-registro-admin')?.setAttribute('hidden', '');
            toast('Registro exitoso. Revisa tu correo para confirmar.', 'ok');
            // Limpiar campos
            ['reg-admin-email', 'reg-admin-password', 'reg-admin-confirm'].forEach(id => {
...
function renderTabla(filtro = '') {
    ...
    const lista = filtro
        ? productos.filter(p =>
            ...
            p.vendedor?.toLowerCase().includes(filtro))
        : productos;
        ...
        <table>
            ...
            <tbody>
                ${lista.map(p => {
                    ...
                    const varBadge    = p.variantes?.length > 0
                        ? `<span class="badge badge-variant">${p.variantes.length} var.</span>`
                        : '<span style="color:#bbb">—</span>';
                    const ofertaBadge = p.enoferta && p.preciooferta
                        ? `<span class="badge" style="background:#ffebee;color:#c62828">$${Number(p.preciooferta).toLocaleString('es-CU')}</span>`
                        : '<span style="color:#bbb">—</span>';
                    return `
                        <tr>
                            <td>
                                <img src="${esc(p.imagen)}" class="prod-thumb"
    ...
    tablaDiv.querySelectorAll('[data-edit]').forEach(btn =>
        btn.addEventListener('click', () => abrirEditar(btn.dataset.edit)));
    tablaDiv.querySelectorAll('[data-del]').forEach(btn =>
        btn.addEventListener('click', () => abrirBorrar(btn.dataset.del, btn.dataset.nombre)));
}
...
function renderPedidos() {
            ...
            <tbody>
                ${pedidos.map(p => `
                    <tr>
                        ...
                        <td><span class="badge ${p.status === 'pendiente' ? 'badge-low' : 'badge-ok'}">
                            ${p.status === 'pendiente' ? 'Pendiente' : 'Confirmado'}
                        </span></td>
                        <td><ul>${(p.productos ?? []).map(prod =>
                            `<li>${esc(prod.nombre)} x${Number(prod.cantidad)} - $${Number(prod.precio * prod.cantidad).toLocaleString('es-CU')}</li>`
                        ).join('')}</ul></td>
                        <td>${p.status === 'pendiente'
                            ? `<button class="act-btn confirmar-pedido" data-id="${esc(p.id)}">Confirmar</button>`
                            : '—'}</td>
                    </tr>
                `).join('')}
            </tbody>
    ...
    tablaDiv.querySelectorAll('.confirmar-pedido').forEach(btn =>
                ...
                <tbody>
                    ${data.map(r => `
                        <tr>
                            ...
                            <td style="color:#ff9800;letter-spacing:2px">${'★'.repeat(Number(r.estrellas))}${'☆'.repeat(5 - Number(r.estrellas))}</td>
                            <td style="max-width:220px;font-size:0.85rem">${esc(r.texto)}</td>
                            <td style="font-size:0.8rem;color:var(--muted);white-space:nowrap">
                                ${new Date(r.fecha).toLocaleDateString('es-CU', { day: 'numeric', month: 'short', year: 'numeric' })}
                            </td>
                            <td><button class="act-btn del" data-del-resena="${esc(r.id)}"><i class="fas fa-trash"></i></button></td>
                        </tr>
                    `).join('')}
                </tbody>
            </table>
        ...
        tablaResenas.querySelectorAll('[data-del-resena]').forEach(btn => {
            btn.addEventListener('click', async () => {
                ...
                try {
                    ...
                    console.error('[admin] eliminarResena:', err);
